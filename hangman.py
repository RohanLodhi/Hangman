...
def loadWords():
    ...
    with open(WORDLIST_FILENAME, newline='') as inputfile:
...
def category(option):
    if option == 1:
            chosen_category = "countries.txt"

    elif option == 2:
            chosen_category = "food.txt"

    elif option == 3:
            chosen_category = "randomwords.txt"
...
if __name__ == "__main__":
    ...
    while choice == "yes":
        option = int(input("What category? \n Enter 1 for countries \n Enter 2 for food items \n Enter 3 for random words \n"))
        chosen_category = category(option)
        WORDLIST_FILENAME = chosen_category
        wordlist = loadWords()
        secretWord = str(chooseWord(wordlist))
        secretWord.lower()
        hangman(secretWord)
